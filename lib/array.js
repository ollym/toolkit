...
extend(Array.prototype, {
  ...
  chunk: function chunk(size) {
     ...
     * @returns array
     */
    if (arguments.length === 0) size = 1;
    
    var arr = [];
    
    for (var i = 0, length = this.length; i < length; i += size)
  ...
  flatten: function flatten(level) {
     ...
     * @returns array
     */
    if (level === 0) return this.slice();
      else if (arguments.length === 0) 
        for (var i = this.length -1, max; i > 0; i--)
          if (Array.isArray(this[i]) && (!level || this[i].length > level))
            level = this[i].length + 1;

    for (var a = this.slice(); level > 0; level--)
  ...
  pluck: function pluck(prop) {
     ...
     */
    return this.map(function (val) {
      return Array.isArray(prop) ? Object.filter(val, prop) : val[prop];
    });
  },
